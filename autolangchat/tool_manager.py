...
``ToolManager`` is injected into the LangGraph ``tools_node`` which calls it
during the tool-call loop.  ``websocket_handler.py`` no longer owns tool execution.
"""
...
class ToolsGenerator:
    ...
    def generate_langchain_tools(self, tool_manager: Optional["ToolManager"] = None) -> List[Any]:
        """Return LangChain ``BaseTool`` objects built from the OpenAPI spec.

        Each tool wraps one OpenAPI endpoint.  The async implementation calls
        ``tool_manager._execute_single_tool_call()`` so all existing HTTP and
        authentication logic is reused.

        When ``tool_manager`` is ``None`` the tools are created but will return
        an error string at call time (useful for schema inspection / testing).

        Args:
            tool_manager: ``ToolManager`` instance whose HTTP client will be
                used to execute the tools.  Can be ``None`` for schema-only use.
        ...
        _JSONTYPE_TO_PYTHON: Dict[str, type] = {
            "string": str,
            "integer": int,
            "number": float,
            "boolean": bool,
        ...
        def _make_structured_tool(func_name: str, tool_meta: Dict, fn_desc: Dict, mgr) -> Any:
            ...
            for p_name, p_schema in properties.items():
                py_type = _JSONTYPE_TO_PYTHON.get(p_schema.get("type", "string"), str)
                description = p_schema.get("description", p_name)
                if p_name in required_params:
                    field_defs[p_name] = (py_type, Field(..., description=description))
                else:
                    from typing import Optional as _Opt

                    field_defs[p_name] = (_Opt[py_type], Field(None, description=description))

            ArgsModel = create_model(f"{func_name}_args", **field_defs)

            async def _invoke(**kwargs: Any) -> str:
                if mgr is None:
                    return _json.dumps({"error": "No tool_manager provided — cannot execute tool"})
        ...
        for func_name, tool_meta in self._generated_tools.items():
            fn_desc = tool_meta["function_desc"]
            tools.append(_make_structured_tool(func_name, tool_meta, fn_desc, tool_manager))

        logger.info("Generated %d LangChain tools from OpenAPI spec", len(tools))
        return tools
...
class ToolManager:
    ...
    def __init__(
        ...
        *,
        ...
        # Cache LangChain tool objects — built once, reused on every message
    ...
    def refresh_tools(self) -> None:
        ...
        self._langchain_tools = None  # invalidate LangChain tool cache
    ...
    def generate_langchain_tools(self) -> List[Any]:
        ...
        Convenience wrapper around ``ToolsGenerator.generate_langchain_tools``
        that automatically supplies ``self`` as the executing ``ToolManager``.

        Returns:
            List of LangChain ``StructuredTool`` instances backed by this
            ``ToolManager``'s HTTP client.
        """
        if self._langchain_tools is None:
            self._langchain_tools = self._generator.generate_langchain_tools(tool_manager=self)
    ...
    async def _execute_single_tool_call(
        ...
        request_kwargs["headers"] = {
            ...
            "User-Agent": "autolangchat/internal",
        }
...
def create_tools_generator_from_spec(
    ...
    **config_kwargs,
    ...
    Example:
        ...
        # Generate tool descriptions — pass to ToolManager or plugin config
