...
This module provides ``ToolsGenerator`` which converts FastAPI routes or standalone
OpenAPI specs into an OpenAI-style tool description schema and LangChain tools.
``ToolManager`` in ``manager.py`` handles HTTP execution only.

Schema generation is **eager**: ``ToolsGenerator`` compiles and caches the
OpenAPI schema immediately on instantiation so every ``ToolManager`` created
from the same generator shares one canonical schema built at service start.
...
Usage (standalone)::
    ...
    config = ChatConfig()
    generator = ToolsGenerator(app=my_fastapi_app, config=config)
    manager = ToolManager(
        generated_tools=generator._generated_tools,
        config=config,
        base_url=generator.get_api_base_url(),
    )
    # LangChain tools — pass manager's executor so HTTP context is bound
    lc_tools = generator.generate_langchain_tools(manager._execute_single_tool_call)

    print(generator.tools_desc)
    print(len(generator.tools_desc["functions"]), "tools")
"""
...
class ToolsGenerator:
    ...
    def __init__(
        ...
        # Single source of truth: {func_name: {path, method, operation, function_desc}}
        self._generated_tools: Optional[Dict[str, Any]] = None
        ...
        # Compile tool schema eagerly so tools_desc is always available for all sessions
        self.generate_tools_desc()

    def generate_tools_desc(self) -> Dict[str, Any]:
        """Build and cache the OpenAPI-derived tool description schema.

        Idempotent — returns the cached result on every call after the first.
        Called automatically by ``__init__``; call again (or use
        ``invalidate_cache()``) only if you need to pick up runtime route changes.

        Returns:
            Dict with ``{"type": "function", "functions": [...]}``, ready for
            direct use as the OpenAI tool-calling payload.
        """
        ...
        try:
            ...
            logger.info("Generated %d tool descriptions from OpenAPI spec", len(self._generated_tools))
            return self.tools_desc
    ...
    @property
    def tools_desc(self) -> Dict[str, Any]:
        """OpenAI-style tool schema, computed from ``_generated_tools``."""
        generated = self._generated_tools or {}
        return {"type": "function", "functions": [t["function_desc"] for t in generated.values()]}

    def invalidate_cache(self) -> None:
        """Invalidate all cached schema data and rebuild immediately.

        Use this when routes change at runtime (e.g. dynamic route registration)
        to force the next ``generate_tools_desc()`` call to re-parse the spec.
        Immediately rebuilds so ``tools_desc`` is never transiently None.
        """
        self._generated_tools = None
        self._openapi_schema = None
        self.generate_tools_desc()
        logger.info("Tool schema invalidated and rebuilt")
    ...
    def _create_function_description(self, path: str, method: str, operation: Dict) -> Optional[Dict]:
        """Create function description for Tool calling"""
...
def create_tools_generator_from_spec(
    ...
    """Create a ToolsGenerator from an OpenAPI spec without a FastAPI app.

    Convenience function for standalone tool generation from a spec file
    or dict.

    Args:
        openapi_spec: Path to OpenAPI spec file (JSON/YAML) or spec dict.
        config: Optional ChatConfig with tool generation preferences.

    Returns:
        ToolsGenerator instance ready to generate tools.
    """
    return ToolsGenerator(openapi_spec=openapi_spec, config=config)
