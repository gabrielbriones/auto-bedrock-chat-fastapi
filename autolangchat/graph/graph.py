...
def build_chat_graph(config: "ChatConfig", tool_manager: Optional["ToolManager"] = None):
    ...
    tool_manager:
        ``ToolManager`` instance.  Injected into every node via the closure
        wrapper so the tools node can execute HTTP calls.
    ...
    logger.info(
        "LangGraph chat graph compiled (nodes: rag → preprocess → llm → [tools] → llm → END, " "checkpointer: %s)",
