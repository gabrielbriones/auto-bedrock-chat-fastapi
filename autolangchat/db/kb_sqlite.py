...
Implements :class:`~autolangchat.db.kb_base.BaseKBStore` using
...
class SQLiteKBStore(BaseKBStore):
    ...
    def _sanitize_fts5_query(query: str) -> str:
        ...
        Strips punctuation and characters that FTS5 interprets as
        syntax operators, then joins remaining words with OR so that
        natural-language queries match documents containing *any* of
        the terms (FTS5 default is implicit AND, which is too strict
        for conversational user input).
        ...
        # Replace punctuation with spaces so tokens like "directly,"
        # or "don't" do not reach FTS5 as malformed barewords.
