        ...
        try:
            ...
            if action == SynthesisAction.SKIP and existing_doc is not None:
                ...
                if embedding_client is None:
                    raise RuntimeError("FeedbackSynthesizer requires embedding_client to re-embed updated KB documents")
                await self._embed_and_add_chunks(doc_id, doc_content, kb_store, embedding_client)
                ...
                if embedding_client is None:
                    raise RuntimeError(
                        "FeedbackSynthesizer requires embedding_client to embed newly created KB documents"
                    )
                await self._embed_and_add_chunks(doc_id, doc_content, kb_store, embedding_client)
