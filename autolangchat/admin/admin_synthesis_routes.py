...
def register_admin_synthesis_routes(
    ...
    *,
    ...
    synthesizer:
        ...
        When ``None``, a default instance is created — ``chat_config`` must
        have been set on it (or ``model_id`` passed at construction) or
        synthesis will raise at call time.
    embedding_client:
        Wired embedding client (``BedrockEmbeddingClient``) for generating
        chunk embeddings.
    """
    ...
    async def trigger_synthesis(identity=Depends(require_admin)) -> SynthesisStatus:  # noqa: B008
        ...
        asyncio.create_task(_run_batch(synth, feedback_store, kb_store, embedding_client, state))
...
async def _run_batch(
    ...
    try:
        result = await synth.synthesize_all(feedback_store, kb_store, embedding_client=embedding_client)
