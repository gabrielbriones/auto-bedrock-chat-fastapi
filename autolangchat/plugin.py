"""Main plugin class and decorator function for autolangchat"""
...
def _load_sso_imports():
    ...
    try:
        from .sso.sso_handler import SSODiscoveryError as _SSODiscoveryError
...
def _setup_logging(config: ChatConfig):
    ...
    logging.getLogger("autolangchat").setLevel(log_level)  # package logger
...
class AutoLangChatPlugin:
    """Main plugin class for integrating AI chat with FastAPI"""
    ...
    def __init__(self, app: FastAPI, config: Optional[ChatConfig] = None, **config_overrides):
        ...
        from .admin.admin_auth import build_admin_authorizer
        ...
        logger.info(f"AutoLangChat Plugin initialized with model: {self.config.model_id}")
    ...
    def _setup_routes(self):
        ...
        async def chat_health():
            """Health check for autolangchat service"""
            try:
        ...
        async def websocket_chat(
            ...
            Pass ``?session_id=<uuid>`` to reconnect to an existing checkpoint
            and resume conversation history (requires Postgres checkpointer).
            """
        ...
        try:
            ...
            if kb_stats_error is not None:
                ...
                if self.config.kb_allow_empty:
                    ...
                    raise AutoLangChatError(error_msg)
                return
            ...
            if self.config.kb_populate_on_startup:
                ...
                if not config_exists:
                    ...
                    if not self.config.kb_allow_empty:
                        raise AutoLangChatError(f"RAG is enabled but KB configuration not found: {config_path}")
                else:
                ...
                logger.warning(
                    ...
                    "   Run: python -m autolangchat.commands.kb populate"
                )
            else:
                error_msg = (
    ...
    def _setup_admin_routes(self):
        ...
        * 403 if the authenticated user is not recognized as an admin
        ...
        from .admin.admin_errors import register_admin_error_handlers
        ...
        if getattr(self, "_feedback_store", None) is not None:
            from .admin.admin_feedback_routes import register_admin_feedback_routes
        ...
        if getattr(self, "_kb_store", None) is not None:
            ...
            logger.info(
                "Admin KB routes skipped: kb_store is not configured " "(ENABLE_RAG=false or backend init failed)"
            )
    ...
    def _setup_shutdown(self):
        ...
        if not hasattr(self.app.state, "autolangchat_cleanup_handlers"):
            self.app.state.autolangchat_cleanup_handlers = []

        # Add our shutdown handler to the app state
        self.app.state.autolangchat_cleanup_handlers.append(self.shutdown)
...
def add_autolangchat(
    ...
    **kwargs,
    ...
    """
    Add AI chat capabilities to a FastAPI application
    ...
    Returns:
        AutoLangChatPlugin instance

    Raises:
        ConfigurationError: If configuration is invalid
        AutoLangChatError: If plugin initialization fails
    ...
    try:
        ...
        plugin = AutoLangChatPlugin(app, **config_overrides)

        return plugin

    except Exception as e:
        logger.error(f"Failed to add autolangchat to FastAPI app: {str(e)}")
        raise AutoLangChatError(f"Plugin initialization failed: {str(e)}")


def create_fastapi_with_autolangchat(**kwargs) -> tuple[FastAPI, AutoLangChatPlugin]:
    """
    Create a new FastAPI app with Bedrock chat plugin using modern lifespan handlers.

    This is the recommended way to create a new FastAPI app with autolangchat support.
    ...
    Returns:
        Tuple of (FastAPI app, AutoLangChatPlugin instance)
    ...
    async def lifespan(app: FastAPI):
        ...
        if hasattr(app.state, "autolangchat_plugin"):
            await app.state.autolangchat_plugin.shutdown()

    # Create FastAPI app with lifespan
    app = FastAPI(lifespan=lifespan)

    # Add autolangchat plugin
    plugin = add_autolangchat(app, **kwargs)

    # Store plugin reference for cleanup
    app.state.autolangchat_plugin = plugin
