...
| `AUTOCHAT_AUTH_EXPIRATION_BEHAVIOUR` | `none`    | How to handle credential expiration mid-session: `none` (legacy — expired tool calls surface a plain HTTP 401 to the LLM), `proactive` (check the SSO access token's expiry at each message boundary and silently refresh it, or send `auth_expired` if no refresh path exists), `reactive` (intercept a tool call's 401, refresh once, and retry, or send `auth_expired` on failure), `both` (apply both checks). Manual bearer tokens have no server-side refresh path and always go straight to `auth_expired` under `proactive`/`reactive`/`both`. See [sso.md](sso.md#session-expired-vs-auth-expiration-handling) for details. |
