...
| ------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `id`          | ❌       | Stable identifier used by [deep links](#deep-linking--auto-start-via-query-string) (`?prompt=<id>`). Auto-generated by slugifying `label` when omitted (e.g. "Health Check" → `health-check`); duplicates get a `-2`, `-3`, … suffix |
...
## Deep Linking / Auto-Start via Query String
...
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `prompt`      | The preset prompt's `id` (explicit or auto-generated slug, see above). Required to trigger a deep link.                                                                                                                                                                                                                                                                                                                                                                                |
| `<VAR_NAME>`  | Any variable name matching `{{VAR_NAME}}` in a template (e.g. `JOB_ID=abc123`); pre-fills the matching input                                                                                                                                                                                                                                                                                                                                                                           |
| `autosend`    | Optional. Set to `0` or `false` to only pre-fill the variable inputs without sending automatically (default: sends as soon as all of the selected prompt's required variables are valid)                                                                                                                                                                                                                                                                                               |
| `auth_method` | Optional. Skips the extra click needed to reach the right auth option when the tool-auth modal is shown. `sso` automatically triggers the "Login with SSO" redirect (identical to clicking it manually — no credentials pass through the URL). Any other supported type name (`bearer_token`, `basic_auth`, `api_key`, `oauth2_client_credentials`, `custom`) only pre-selects that option in the dropdown; the user still enters and submits the actual credential values themselves. |

The link is resolved client-side once the chat becomes usable (i.e. after any required authentication completes) and only ever applied once per page load — reconnects don't re-trigger it.

### Handling authentication

Deep links never carry credentials (bearer tokens, client secret, API keys, etc.) in the URL — putting secrets in a querystring is unsafe: they end up in browser history, server access logs, proxy logs, and the `Referer` header sent to third parties. Instead:

- **No auth / tool-auth not required**: the deep link applies as soon as the WebSocket connects.
- **Tool-auth modal (bearer token, basic auth, API key, OAuth2 client credentials, custom headers)**: this is an in-page modal, not a redirect, so the page URL — and therefore the deep-link querystring — is untouched while the user fills it in. Once the server confirms `auth_configured`, the deep link is applied automatically. `auth_method=<type>` only pre-selects the dropdown entry; it never pre-fills credential values.
- **SSO**: logging in navigates the browser away to the identity provider and back, so the querystring would normally be lost. The chat UI passes the current path and query string as a `next` parameter to `/chat/auth/sso/login`; the server validates it's a same-site relative path (rejecting absolute/protocol-relative URLs to prevent open-redirect) and redirects back to it after the SSO callback completes and the `HttpOnly` session cookie is set. No token or credential is ever placed in the URL — only the (non-secret) original chat page path/query. `auth_method=sso` automates clicking the "Login with SSO" button so a deep link can complete the whole SSO round trip with zero manual interaction (assuming the user already has an active session with the identity provider).
